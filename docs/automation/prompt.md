...
- 若更新 `insights/layer-*.yaml`：只保留与本层主营业务 **强相关** 的标的，不能因为“AI 可能提升效率”就把白酒、银行、传统消费龙头等泛价值股放进 AI 分层
- 若更新 `web/src/config/layers.ts` 中候选池：优先替换成更贴近该层的 AI 标的，而不是保留防御型或主题弱相关样本
