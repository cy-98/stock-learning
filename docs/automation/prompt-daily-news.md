...
| id | 名称 | 覆盖主题 |
|----|------|----------|
| 1 | 算力与基础设施层 | 半导体、GPU/HBM、晶圆代工、设备、光模块、IDC、电力 |
| 2 | 模型与算法层 | 大模型、多模态、开源生态、对齐与安全、端侧 AI |
| 3 | 平台与工具层 | 云计算、MLOps、RAG、企业软件、数据工程、AI 安全 |
| 4 | 应用与解决方案层 | 行业 Copilot、金融/制造/消费等垂直 AI、SaaS ROI |
| 5 | 终端与生态层 | 机器人、智能汽车、稀土、电力公用、主题与政策 |
...
- **股票代码**：优先用仓库候选池已有代码；美股 `usNVDA`、港股 `hk0700` 等保持与 `layers.ts` 一致
